...
int copyFolderFiles(char* folderFrom, char* folderTo) {
    ...
    fnFrom = (char*)malloc(sizeof(char)*MAXDIRSIZE);
    fnTo = (char*)malloc(sizeof(char)*MAXDIRSIZE);
    //printf("folderFrom: %s\n", folderFrom);
    ...
    while((file = readdir(fdir)) != NULL) { 
        strcpy(fnFrom, folderFrom);
        strcat(strcat(fnFrom, "/"), file->d_name);
    	strcpy(fnTo, folderTo);
        strcat(strcat(fnTo, "/"), file->d_name);
        ...
        if(((sbuf->st_mode) & S_IFMT) == S_IFREG){
            ...
            //printf("unlinked: %s\n", fnTo);
            //s = sbrk(strlen(file->d_name) + 1);
            //memcpy(s, file->d_name, strlen(file->d_name));
        }
    }
    closedir(fdir);
    //brk(start);
